...
mod SportsPool {
    ...
    struct tournament_instance {
        ...
        prize_second_place: felt252,
        prize_third_place: felt252,
        prize_first_place_claimed: bool,
        prize_second_place_claimed: bool,
        prize_third_place_claimed: bool,
    }
    ...
    impl SportsPoolImpl of ISportsPool {
        ...
        fn save_tournament_instance(ref self: ContractState, tournament_instance: tournament_instance) {
            ...
            let mut updated_tournament_instance = tournament_instance {
                ...
                prize_first_place_claimed: false,
                prize_second_place_claimed: false,
                prize_third_place_claimed: false,
            };
        ...
        fn _create_leaderboard(self: @ContractState, tournament_instance_id: u8) -> Array<UserPoints> {
            // Get tournament_template_id from the instance
            let tournament_template_id = self._tournament_instance.entry(tournament_instance_id).tournament_template_id.read();
        ...
        fn claim_prize(ref self: ContractState, tournament_instance_id: u8) {
            ...
            // Check if sender in top 3
            assert!(leader1.user == msg_sender || leader2.user == msg_sender || leader3.user == msg_sender, "Sender not in top 3");
            ...
            // Then prize will depend on the position of the msg sender
            let mut prize: u256 = 0;
            if leader1.user == msg_sender {
                // check if 1st place is already claimed
                assert!(!self._tournament_instance.entry(tournament_instance_id).prize_first_place_claimed.read(), "1st place already claimed");
                // get the prize of the tournament instance and set 1st place as claimed
                prize = self._tournament_instance.entry(tournament_instance_id).prize_first_place.read().into();
                self._tournament_instance.entry(tournament_instance_id).prize_first_place_claimed.write(true);
            } else if leader2.user == msg_sender {
                // check if 2nd place is already claimed
                assert!(!self._tournament_instance.entry(tournament_instance_id).prize_second_place_claimed.read(), "2nd place already claimed");
                // get the prize of the tournament instance and set 2nd place as claimed
                prize = self._tournament_instance.entry(tournament_instance_id).prize_second_place.read().into();
                self._tournament_instance.entry(tournament_instance_id).prize_second_place_claimed.write(true);
            } else if leader3.user == msg_sender {
                // check if 3rd place is already claimed
                assert!(!self._tournament_instance.entry(tournament_instance_id).prize_third_place_claimed.read(), "3rd place already claimed");
                // get the prize of the tournament instance and set 3rd place as claimed
                prize = self._tournament_instance.entry(tournament_instance_id).prize_third_place.read().into();
                self._tournament_instance.entry(tournament_instance_id).prize_third_place_claimed.write(true);
            }
            let final_prize = (entry_fee * total_participants.into()) * prize / 100;
            // transfer the prize to the msg sender
            let strk_token_dispatcher = ERC20ABIDispatcher {
                contract_address: contract_address_const::<
                    0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d,
                >() // STRK Contract Address
            };
            strk_token_dispatcher.transfer(msg_sender, final_prize);
        }
        ...
        fn check_prize(self: @ContractState, tournament_instance_id: u8, user_address: ContractAddress) -> u256 {
            ...
            let entry_fee = self._tournament_instance.entry(tournament_instance_id).entry_fee.read();
            let total_participants = self._user_instances_user_list_count.entry(tournament_instance_id).read();
            // Then prize will depend on the position of the user
            let mut prize: u256 = 0;
            if leader1.user == user_address {
                // check if 1st place is already claimed
                assert!(!self._tournament_instance.entry(tournament_instance_id).prize_first_place_claimed.read(), "1st place already claimed");
                // get the prize of the tournament instance
                prize = self._tournament_instance.entry(tournament_instance_id).prize_first_place.read().into();
            } else if leader2.user == user_address {
                // check if 2nd place is already claimed
                assert!(!self._tournament_instance.entry(tournament_instance_id).prize_second_place_claimed.read(), "2nd place already claimed");
                // get the prize of the tournament instance
                prize = self._tournament_instance.entry(tournament_instance_id).prize_second_place.read().into();
            } else if leader3.user == user_address {
                // check if 3rd place is already claimed
                assert!(!self._tournament_instance.entry(tournament_instance_id).prize_third_place_claimed.read(), "3rd place already claimed");
                // get the prize of the tournament instance
                prize = self._tournament_instance.entry(tournament_instance_id).prize_third_place.read().into();
        ...
        #[external(v0)]
        fn get_user_instances_count(self: @ContractState, tournament_instance_id: u8) -> u8 {
            self._user_instances_user_list_count.entry(tournament_instance_id).read()
        }
